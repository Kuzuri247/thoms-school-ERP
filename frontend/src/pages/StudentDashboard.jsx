...
const StudentDashboard = ({ activeTab = "home" }) => {
  ...
  const fetchStudentData = async () => {
    try {
      ...
      const [workRes, noticesRes, feeRes, attRes, elearnRes] =
        await Promise.all([
          api
            .get("/homework/student/my-work")
            .catch(() => ({ data: { data: [] } })),
          api
            .get("/notices/student-work")
            .catch(() => ({ data: { data: [] } })),
          api
            .get("/payments/records/my-fees")
            .catch(() => ({ data: { data: [] } })),
          api
            .get("/attendance/student/my-summary")
            .catch(() => ({ data: { data: null } })),
          api
            .get("/elearning/student/my-learning")
            .catch(() => ({ data: { data: [] } })),
        ]);
  ...
  const feeStatusBadge =
    pendingAmount === 0 ? "PAID" : pendingAmount > 5000 ? "OVERDUE" : "PENDING";
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 text-indigo-100 font-medium text-xs sm:text-sm">
              ...
              <p>
                Role: <span className="font-bold text-white">Student</span>
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-3">
              ...
              <p className="text-xs text-slate-500 font-medium">
                Updated fee status and transaction record.
              </p>
            </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-3">
              ...
              <h3 className="text-2xl font-black text-slate-900">
                {workItems.length + workNotices.length} Tasks
              </h3>
              <p className="text-xs text-slate-500 font-medium">
                Homework tasks & academic notices assigned.
              </p>
            </div>
          ...
          <div className="flex items-center justify-between border-b border-slate-100 pb-4">
            <div>
              <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                <BookText className="w-5 h-5 text-indigo-600" /> My Academic
                Work & Homework
              </h2>
          ...
          <div className="space-y-4">
            ...
            {workNotices.map((notice) => (
              <div
                ...
                className="p-4 rounded-2xl bg-indigo-50/60 border border-indigo-100 space-y-1"
              >
                <div className="flex items-center justify-between">
            ...
            {workItems.map((item) => {
              ...
              return (
                <div
                  key={`hw-${item.homework_id || item.id}`}
                ...
                >
                      ...
                      <button
                        onClick={async () => {
                          const newStatus = isCompleted
                            ...
                            : "completed";
                          try {
                            await api.put("/homework/status", {
                              homework_id: item.homework_id || item.id,
                              status: newStatus,
                            });
                            setWorkItems((prev) =>
                              prev.map((w) =>
                                (w.homework_id || w.id) ===
                                (item.homework_id || item.id)
                                  ? { ...w, status: newStatus }
                                  : w,
                              ),
                            );
                  ...
                  {item.classroom_url && (
                    <p className="text-[10px] text-emerald-700 font-semibold mt-1 flex items-center gap-1">
                      💡{" "}
                      <span>
                        Submit work on Google Classroom first, then click{" "}
            ...
            {workItems.length === 0 && workNotices.length === 0 && (
              <p className="text-xs text-slate-400 font-medium py-4 text-center">
                No pending work or coursework assigned at this time.
              </p>
            )}
          </div>
        </div>
      ...
      {(currentTab === "home" || currentTab === "timetable") && (
        <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs space-y-4 animate-in fade-in duration-200">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                <Clock className="w-5 h-5 text-indigo-600" /> Class Schedule &
                Timetable (7 Periods)
              </h2>
              <p className="text-xs text-slate-500 font-semibold mt-0.5">
                Standard 7-period daily schedule with Recess Break after 3rd
                Period.
              </p>
            </div>
          </div>
                ...
                >
                  ...
                  <div className="space-y-1.5">
                    {defaultSlots.map((slot) => {
                      const item = dayClasses.find(
                        (t) => Number(t.period_no) === slot.p,
                      );
                          ...
                          <div className="p-2 bg-white rounded-xl border border-slate-100 flex items-center justify-between">
                            <div>
                              <span className="text-xs font-bold text-slate-900 block">
                                P{slot.p}:{" "}
                                {item ? item.subject_name : "Study Period"}
                              </span>
                              {item?.teacher_name && (
      ...
      {(currentTab === "home" || currentTab === "fees") && (
        <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs space-y-4 animate-in fade-in duration-200">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                <CreditCard className="w-5 h-5 text-indigo-600" /> Fee Status &
                Recent Transactions
              </h2>
              <p className="text-xs text-slate-500 font-medium mt-1">
                Your fee account balance and payment history.
              </p>
            </div>
            <span
                  ...
                  fees.map((f, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/60">
                      ...
                      <td className="py-3 px-4 text-right font-black text-slate-800">
                        ₹{Number(f.paid_amount || 0).toLocaleString()} / ₹
                        {Number(f.total_amount || 0).toLocaleString()}
                      </td>
                    </tr>
                  ))
                )}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
            <div>
              <h2 className="text-xl font-black text-slate-900 flex items-center gap-2">
                <Tv className="w-5 h-5 text-indigo-600" /> E Learning Video
                Portal
              </h2>
              <p className="text-xs text-slate-500 font-medium mt-1">
                Watch video tutorials & topic explanations shared by your
                subject teachers.
              </p>
            </div>
          ...
          {(() => {
              ...
              return (
                <div className="py-12 text-center text-slate-400 space-y-2">
                  ...
                  <p className="text-xs font-bold text-slate-600">
                    No E Learning video topics found
                  </p>
                  <p className="text-[11px] text-slate-400">
                    {elearningSearch
                      ? "No videos match your search query."
                      : "Your teachers have not shared any E Learning video topics for your section yet."}
                  </p>
                </div>
              );
            }
            ...
            return (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filtered.map((item) => {
                  const ytId =
                    item.youtube_video_id || extractYouTubeId(item.youtube_url);
                  const isApprovedUrl =
                    typeof item.youtube_url === "string" &&
                    /^https?:\/\//i.test(item.youtube_url.trim());
                  return (
                    <div
                      key={item.id}
                      className="bg-slate-50 p-4 rounded-3xl border border-slate-200/80 space-y-3 flex flex-col justify-between hover:border-slate-300 transition shadow-2xs"
                    >
                      ...
                      {ytId ? (
                        ...
                        >
                          <ExternalLink className="w-4 h-4" /> Watch Video on
                          YouTube
                        </a>
                      ) : null}
                    </div>
