    ...
    <div className="space-y-6">
          ...
          {(user?.role === 'super_admin' || user?.role === 'admin') && (
            <>
              ...
              <button
                ...
                className="flex items-center gap-2 px-3.5 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-xl text-xs font-bold shadow-md hover:shadow-violet-500/20 transition cursor-pointer"
              >
                <UserPlus className="w-4 h-4" />
                Add Staff
              </button>
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                    {cls.sections.map(sec => (
                      <span
                        key={sec.section_id}
                        onClick={(e) => { e.stopPropagation(); setSelectedClass(cls); setSelectedSection(sec); }}
      ...
      {showAddStaffModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 border border-slate-200 animate-in fade-in zoom-in-95 my-8">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-base font-black text-slate-900">Add New Staff Profile</h3>
                  <p className="text-[11px] font-semibold text-slate-400">Enroll new teacher, bus staff, or admin profile</p>
                </div>
              </div>
              <button onClick={() => setShowAddStaffModal(false)} className="p-1.5 text-slate-400 hover:text-slate-600 rounded-lg hover:bg-slate-100 transition cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>

            <form onSubmit={handleAddStaffFrontend} className="space-y-3.5 text-xs font-semibold text-slate-700">
              {/* Photo Upload Box */}
              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200 flex items-center gap-3">
                <div className="w-12 h-12 rounded-xl bg-white border border-slate-300 overflow-hidden flex items-center justify-center flex-shrink-0">
                  {staffForm.profile_pic ? (
                    <img src={staffForm.profile_pic} alt="Staff Preview" className="w-full h-full object-cover" />
                  ) : (
                ...
                <div className="flex-1">
                  ...
                  <label className="inline-flex items-center gap-1.5 px-3 py-1 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-xs font-bold transition cursor-pointer">
                    <Upload className="w-3.5 h-3.5" />
                    Browse Photo
                    <input
                      type="file"
                      ...
                      onChange={(e) => {
                        ...
                        if (file) {
                          const reader = new FileReader();
                          reader.onloadend = () => setStaffForm(prev => ({ ...prev, profile_pic: reader.result }));
                          reader.readAsDataURL(file);
                        }
                      }}
                      className="hidden"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-violet-500 outline-none font-bold text-slate-800"
                />
              </div>

              <div>
                <label className="block mb-1">Assigned Designation / Role *</label>
                <select
                ...
                >
                  ...
                  <option value="busstaff">Bus Staff / Transport</option>
                  <option value="staff">Non-Teaching Staff (Librarian / Guard)</option>
                </select>
              </div>

              <div>
                <label className="block mb-1">Department / Subject Area</label>
                <input
                  type="text"
                  placeholder="e.g. Mathematics Department / Senior Science"
              ...
              <div className="pt-2 border-t border-slate-100 flex justify-end gap-2">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-xl text-xs font-bold transition shadow-md shadow-violet-500/20 cursor-pointer"
                >
                  Save & Enroll Staff
                </button>
              </div>
